...
static TO_lib_ret_t sock_send(void *esp_tls, const uint8_t *data, uint32_t len)
{
    ...
    err_tls = esp_tls_get_conn_sockfd(tls, &sockfd);
	if (err_tls != ESP_OK) {
		return TO_ERROR;
	}

	ESP_LOGD(TAG, "sending %lu bytes on socket", len);
					...
					uint32_t *read_len, int32_t timeout)
{
	...
	ESP_LOGD(TAG, "receiving %lu bytes on socket", len);
	ret = recv(sockfd, data, len, 0);
	if (ret < 0) {
		*read_len = 0;
		return TO_ERROR;
	}
...
/**
 * @brief get LibTO ssl context
 * @param[out] tls context
 *
 * @note
 **/
void *esp_libto_get_ssl_context(esp_tls_t *tls)
{
    if (tls == NULL) {
        ESP_LOGE(TAG, "Invalid arguments");
        return NULL;
    }
    return (void*)tls->libto_tls;
}
